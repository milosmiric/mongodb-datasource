...
- **Macro & pipeline-stage autocomplete** in the pipeline editor. Typing `$__` suggests the plugin's macros (`$__timeFilter`, `$__timeFilter_ms`, `$__oidFilter`, `$__timeGroup`, `$__match`) and template variables (`$__from`/`$__to`, the interval/ObjectId/range variables, `$__maxDataPoints`), each with a one-line description; macros insert a snippet with the cursor on the field argument. Common aggregation stage keywords (`$match`, `$group`, `$project`, `$lookup`, `$unwind`, …) are suggested too. Completions are hints only and rank after field names.
